...
# Run via Make target:
B2_BUCKET=immich-backup-df18 make b2-size-check

# Or run directly:
B2_BUCKET=immich-backup-df18 ./scripts/b2_size_check.sh
```
...
## Safe lifecycle policy for Restic bucket
...
- B2 lifecycle cleanup is not always instant.
- If policy is keep only last version without day-based expiration, exact deletion time is not deterministic per object.
- Restic prune can be clean while bucket still has old non-current versions.
